...
def calculate_technical_indicators(df) -> list:
    df = pd.DataFrame(df)
    df = df.iloc[::-1]
    ...
    indicators["SMA_10"] = ta.SMA(df["_close"], 10)
    indicators["SMA_20"] = ta.SMA(df["_close"], 20)
    indicators["SMA_50"] = ta.SMA(df["_close"], 50)
    indicators["SMA_100"] = ta.SMA(df["_close"], 100)
    indicators["SMA_200"] = ta.SMA(df["_close"], 200)
    indicators["EMA_12"] = ta.EMA(df["_close"], 12)
    indicators["EMA_26"] = ta.EMA(df["_close"], 26)
    indicators["EMA_50"] = ta.EMA(df["_close"], 50)
    indicators["EMA_100"] = ta.EMA(df["_close"], 100)
    indicators["WMA_30"] = ta.WMA(df["_close"], 30)
    indicators["HMA_30"] = ta.WMA(df["_close"], 30)  # Approximate Hull Moving Average
    indicators["DEMA_30"] = ta.DEMA(df["_close"], 30)
    indicators["TEMA_30"] = ta.TEMA(df["_close"], 30)

    # RSI (Relative Strength Index)
    indicators["RSI_14"] = ta.RSI(df["_close"], timeperiod=14)
    indicators["RSI_7"] = ta.RSI(df["_close"], timeperiod=7)

    # MACD (Moving Average Convergence Divergence)
    macd, macd_signal, macd_hist = ta.MACD(df["_close"])
    indicators["MACD"] = macd
    indicators["MACD_signal"] = macd_signal
    indicators["MACD_hist"] = macd_hist

    # Stochastic Oscillator
    stoch_k, stoch_d = ta.STOCH(df["_high"], df["_low"], df["_close"])
    indicators["Stoch_K"] = stoch_k
    indicators["Stoch_D"] = stoch_d

    # ATR (Average True Range) - Measures volatility
    indicators["ATR_14"] = ta.ATR(df["_high"], df["_low"], df["_close"], timeperiod=14)

    # ADX (Average Directional Index) - Measures trend strength
    indicators["ADX_14"] = ta.ADX(df["_high"], df["_low"], df["_close"], timeperiod=14)

    # CCI (Commodity Channel Index) - Measures momentum
    indicators["CCI_14"] = ta.CCI(df["_high"], df["_low"], df["_close"], timeperiod=14)

    # Williams %R
    indicators["WILLR_14"] = ta.WILLR(df["_high"], df["_low"], df["_close"], timeperiod=14)

    # Chaikin A/D Line (Accumulation/Distribution)
    indicators["AD"] = ta.AD(df["_high"], df["_low"], df["_close"], df["volume"])

    # On-Balance Volume (OBV)
    indicators["OBV"] = ta.OBV(df["_close"], df["volume"])

    # Parabolic SAR
    indicators["SAR"] = ta.SAR(df["_high"], df["_low"])

    # Momentum Indicator
    indicators["MOM_10"] = ta.MOM(df["_close"], timeperiod=10)

    # ROC (Rate of Change)
    indicators["ROC_10"] = ta.ROC(df["_close"], timeperiod=10)

    # MFI (Money Flow Index) - Measures buying and selling pressure
    indicators["MFI_14"] = ta.MFI(
        df["_high"], df["_low"], df["_close"], df["volume"], timeperiod=14
    )

    # Ultimate Oscillator
    indicators["ULTOSC"] = ta.ULTOSC(df["_high"], df["_low"], df["_close"])

    # TRIX (Triple Exponential Average)
    indicators["TRIX"] = ta.TRIX(df["_close"])

    # Keltner Channel
    kc_middle = ta.EMA(df["_close"], 20)
    kc_upper = kc_middle + 2 * ta.ATR(df["_high"], df["_low"], df["_close"], timeperiod=10)
    kc_lower = kc_middle - 2 * ta.ATR(df["_high"], df["_low"], df["_close"], timeperiod=10)
    indicators["KC_upper"] = kc_upper
    indicators["KC_middle"] = kc_middle
    indicators["KC_lower"] = kc_lower

    # Aroon Indicator
    aroon_up, aroon_down = ta.AROON(df["_high"], df["_low"], timeperiod=14)
    indicators["Aroon_Up"] = aroon_up
    indicators["Aroon_Down"] = aroon_down

    # Donchian Channel
    donchian_upper = df["_high"].rolling(window=20).max()
    donchian_lower = df["_low"].rolling(window=20).min()
    ...
    indicators["CMO_14"] = ta.CMO(df["_close"], timeperiod=14)

    # Stochastic RSI
    stochrsi_k, stochrsi_d = ta.STOCHRSI(df["_close"])
    indicators["Stoch_RSI_K"] = stochrsi_k
    indicators["Stoch_RSI_D"] = stochrsi_d

    # Elder's Force Index
    indicators["EFI_13"] = df["_close"].diff(1) * df["volume"]

    # VWAP (Volume Weighted Average Price)
    vwap = (df["_close"] * df["volume"]).cumsum() / df["volume"].cumsum()
    indicators["VWAP"] = vwap

    # Ichimoku Cloud
    tenkan_sen = (df["_high"].rolling(9).max() + df["_low"].rolling(9).min()) / 2
    kijun_sen = (df["_high"].rolling(26).max() + df["_low"].rolling(26).min()) / 2
    senkou_span_a = (tenkan_sen + kijun_sen) / 2
    senkou_span_b = (df["_high"].rolling(52).max() + df["_low"].rolling(52).min()) / 2
    chikou_span = df["_close"].shift(-26)
    ...
    # Pivot Points
    pivot = (df["_high"] + df["_low"] + df["_close"]) / 3
    r1 = (2 * pivot) - df["_low"]
    s1 = (2 * pivot) - df["_high"]
    r2 = pivot + (df["_high"] - df["_low"])
    s2 = pivot - (df["_high"] - df["_low"])
    ...
    # Vortex Indicator
    vi_plus = ta.PLUS_DI(df["_high"], df["_low"], df["_close"], timeperiod=14)
    vi_minus = ta.MINUS_DI(df["_high"], df["_low"], df["_close"], timeperiod=14)
